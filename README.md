...
- Мониторинг: Spring Boot Actuator + метрики Prometheus.
...
Публичные GET-запросы. Если у запроса есть активная сессия, ответ дополнительно содержит `userData`.

| Метод | Эндпоинт                  | Описание |
|------:|---------------------------|----------|
| GET   | `/games/search?query=...` | Поиск игр |
| GET   | `/games/{igdb-id}`        | Детальная информация об игре |
...
| GET    | `/users/game-lists/{list-id}`              | Получить публичный или собственный список; с `userData` при активной сессии |
...
| Метод | Эндпоинт                  | Описание |
|------:|---------------------------|----------|
| GET   | `/steam-import/{steam-id}`| Получить список игр пользователя Steam с `userData` |
| POST  | `/steam-import/{steam-id}`| Запустить импорт выбранных игр (Kafka) |

### Legacy Enriched API

Эндпоинты оставлены для совместимости. Для нового клиентского кода используйте обычные маршруты выше.
